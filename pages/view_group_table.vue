...
const groups = ref<Array<any>>([
  // Sample group data to avoid errors and for testing
  { id: 1, name: 'Group 1', dateCreated: '2023-01-01', isActive: true },
  { id: 2, name: 'Group 2', dateCreated: '2023-02-01', isActive: false }
]);
const searchQuery = ref<string>('');
const showCreateModal = ref<boolean>(false);
const showUpdateModal = ref<boolean>(false);
const newGroupName = ref<string>('');
const newGroupStatus = ref<string>('true');
const editGroupName = ref<string>('');
const editGroupStatus = ref<string>('true');
const editGroupId = ref<number | null>(null);

// Computed property to filter groups based on search query
...
// Simulated fetchGroups method
const fetchGroups = () => {
  console.log('Fetching groups...');
  // Simulate group fetching without an API
};
...
const addGroup = () => {
  if (newGroupName.value.trim() !== '') {
    const newGroup = {
      id: groups.value.length + 1, // Generate a new ID
      name: newGroupName.value,
      dateCreated: new Date().toISOString().split('T')[0],
      isActive: newGroupStatus.value === 'true',
    };
    groups.value.push(newGroup); // Add the new group to the list
    newGroupName.value = '';
    newGroupStatus.value = 'true';
    showCreateModal.value = false;
  }
};

// Method to open the update modal and set group values for editing
const editGroup = (group: any) => {
  editGroupId.value = group.id;
  editGroupName.value = group.name;
  editGroupStatus.value = group.isActive ? 'true' : 'false';
...
const updateGroup = () => {
  if (editGroupName.value.trim() !== '') {
    const index = groups.value.findIndex(group => group.id === editGroupId.value);
    if (index !== -1) {
      groups.value[index].name = editGroupName.value;
      groups.value[index].isActive = editGroupStatus.value === 'true';
    }
    editGroupName.value = '';
...
// Fetch groups on component mount (simulated here)
